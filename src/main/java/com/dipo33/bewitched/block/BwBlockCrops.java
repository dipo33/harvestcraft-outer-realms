...
public class BwBlockCrops extends BlockCrops {
     ...
     *
     * @param stages the number of growth stages (must be between 1 and 8 inclusive)
     * @return       this BwBlockCrops instance for method chaining
     * @throws IllegalArgumentException if {@code stages} is less than 1 or greater than 8
     */
     ...
     *
     * @param plantType the plant type to assign to this crop; must not be null
     * @return the same BwBlockCrops instance for method chaining
     * @throws NullPointerException if {@code plantType} is null
     */
     ...
     *
     * @param item   an ObjectHolder wrapping the item to add as an extra drop
     * @param chance the probability that the item will be dropped (must be between 0.0 and 1.0 inclusive)
     * @throws NullPointerException     if {@code item} is null
     * @throws IllegalArgumentException if {@code chance} is NaN or is less than 0.0 or greater than 1.0
     * @return this block instance for method chaining
     */
     ...
     *
     * @param side the block face being queried
     * @param meta the block metadata representing the crop's growth stage
     ...
     * registering textures named "<textureName>_stage_<index>".
     *
     * @param reg the icon register used to register stage textures
     */
    @Override
    @SideOnly(Side.CLIENT)
     ...
     *
     * @param metadata the block metadata at the position (used to determine maturity)
     * @param fortune  the Fortune enchantment level affecting base drops
     * @return an ArrayList of ItemStack containing the standard drops plus any extra drops added for a mature crop
     */
    @Override
    ...
    /**
     * Harvests the crop at the given position by dropping its items and resetting its growth when the crop is fully mature and right-click harvesting is enabled.
     *
     * This action occurs only when Config.rightClickMatureCropHarvest is true and the call is executed on the server (world is not remote).
     *
     ...
     * @param subX   relative X hit position on the block face
     * @param subY   relative Y hit position on the block face
     * @param subZ   relative Z hit position on the block face
