...
public class WaterItemSeeds extends ItemSeeds {
     ...
     *
     * @param crop the Block that will be placed as the crop when this seed is planted
     * @param soil the Block representing the required soil type for planting (passed to the superclass)
     */
     ...
     *
     * If the player is targeting a block and has permission to interact, and the targeted block is source water (material water with metadata 0)
     * with an air block above it, this method places the configured crop block on that air block. Placement is reverted if the Forge block-place event is canceled.
     * The item stack is decremented when a seed is planted unless the player is in creative mode.
     *
     * @return the (possibly modified) ItemStack after the right-click action; `stackSize` is decremented when a seed is successfully planted and the player is not in creative mode.
     */
    @Override
